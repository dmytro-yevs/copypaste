...
## Реліз
- **Гілка:** `v0.5.2-fixes` — **HEAD `6ac9460`** (хвиля 3 + фікс-хвиля 1 змерджені; V-13/V-14 мікрофікс мерджиться)
...
- **Гейт зараз:** `cargo check --workspace` PASS · `--features cloud-sync` PASS · `tsc --noEmit` PASS. (Локально; CI ще не тригернуто.)
- **Цикл якості:** 8 верифікаторів знайшли ~20 багів (1 compile-blocker + data-loss ключа + jank/OOM/popup-регресії) → 5 фіксерів виправили → cloud-sync build-break (схований feature-gate) теж виправлено. Далі: re-verify + аудит модулів.
