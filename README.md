...
| Android beyond launch and storage | The nightly emulator run has two legs: a debug x86_64 APK for the filesystem assertions and a release one for R8 and signing, whose key is ephemeral and whose artifact is never published. The debug leg establishes launch, a painted WebView, the Keystore secret surviving a restart and an unreadable SQLCipher file, and a second script (`scripts/release/android-rungs.sh`) adds the four surfaces that were previously asserted only negatively: the Quick Settings tile saves a clip another app copied, `FLAG_SECURE` is on the window from the first dump, no capture service or notification appears when nothing is listening, and rung 2's platform half — a clipboard read as the shell uid with no focus — now runs. **The debug leg last ran on 2026-08-01** (run `30688166417`); every run after it was skipped by the dependency audit failing in the build job ahead of it, and while that gate has since been rescoped ([ADR-0016](docs/adr/0016-the-android-cve-gate-audits-the-apk.md)), no nightly has been observed green since. On a publishable tag, the pipeline checksum-verifies, installs and runs the exact signed universal APK before publication. **Shizuku's own transport is still unverified**: the binder proxy, the reflected `IClipboard` and whether the clip listener fires need a phone paired by hand, as do the Android 12+ toast and OEM battery managers. [`docs/rewrite/android-spike.md`](docs/rewrite/android-spike.md) lists what is settled and what a device run would still falsify. |
| Android on the hardware people own | The emulator is x86_64, stock AOSP, one API level. No aarch64 instruction executes in any run anywhere, so the hardware SHA-2 backend is evidenced only by a static count — 56 SHA-2 instructions in the aarch64 object against 0 on x86_64. No vendor battery manager is applied; the WebView is the one pinned into the system image, not the one Play updates; and no signed release APK has been installed on a phone. [`docs/rewrite/testing-policy.md`](docs/rewrite/testing-policy.md) carries the four as matrix rows, and [`docs/rewrite/android-api-levels.md`](docs/rewrite/android-api-levels.md) says which assertions the API spread is supposed to move. |
