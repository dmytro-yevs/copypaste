...
| Android beyond launch and storage | The emulator matrix installs debug and release x86_64 test APKs. It establishes launch, a painted WebView, Keystore persistence, SQLCipher storage, R8 and signing. Its API 36 leg also proves a shell-uid clipboard read without focus, a Quick Settings tile capture, fail-closed background-service state and `FLAG_SECURE`. Shizuku's binder proxy and clip listener still need a hand-paired phone, as do Android 12+ privacy UI and OEM battery managers; [`docs/rewrite/android-spike.md`](docs/rewrite/android-spike.md) lists the remaining device evidence. |
