...
describe("the bulk bar", () => {
  ...
  test("pins the selection, and then offers to unpin it (CopyPaste-8ebg.55)", async () => {
    ...
    await waitFor(
      async () => (await storedItems(app)).filter((item) => item.pinned).length === 2,
  ...
  test("bulk delete confirms first, then really deletes", async () => {
    ...
    const doomed = (await rowBoxes(app))
      ...
      .slice(0, 2)
      .map((row) => row.text.split(/\r?\n/)[0]!.trim());
    expect(doomed.filter(Boolean)).toHaveLength(2);

    await tapButton(app, "Delete", { within: '[role="alertdialog"]' });

    await waitFor(
      async () => (await storedItems(app)).length === before.length - 2,
      "the store still holds the deleted items",
      20_000,
    );
    await waitFor(
      async () => {
        const shown = await visibleText(app);
        return doomed.every((text) => !shown.includes(text));
      },
      `the deleted rows stayed on screen: ${doomed.join(", ")}`,
      20_000,
    );
  }, 120_000);
