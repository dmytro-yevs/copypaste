...
fun MainShell(viewModel: ClipboardViewModel) {
    ...
    // D7 edge-to-edge backdrop (S5 carried task): CLIPS renders with NO outer
    // bottom padding — its own list threads the reserved bottom space (computed
    // inside [MainShellContent]) down as `HistoryScreen.bottomContentPadding`,
    // which HistoryList applies as
    // LazyColumn `contentPadding` (not a Modifier.padding that would shrink
    // the composable's own drawable bounds). That means History's real
    // content pixels are laid out and scroll BEHIND the floating pill, so
    // the pill's backdrop blur samples real content instead of an empty
    // gradient-only gap.
    //
    // DEVICES/SETTINGS are NOT owned by this wave/slice (S7/S9) and keep the
    // PRE-EXISTING behaviour verbatim: an outer `Modifier.padding` that
    // shrinks their bounds above the reserved space (applied inside
    // [MainShellContent]). Adopting the same contentPadding-based pattern
    // for those two screens is tracked as a follow-up for their owning
    // slices (CopyPaste-myh8.5 bd notes).
    MainShellContent(
        selectedTab = NavTab.entries[selectedTab],
        onTabSelected = { tab ->
