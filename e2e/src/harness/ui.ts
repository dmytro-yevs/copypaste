...
function signature(snapshot: ListSnapshot): string {
    const rows = snapshot.rows
        .map(
            (row) =>
                `${row.id}:${Math.round(row.start)}+${Math.round(row.height)}`,
        )
        .join(",");
    return (
...
export async function waitForRows(
    ...
    await browser.waitUntil(async () => (await rowCount(browser)) >= atLeast, {
...
export async function activeRowId(browser: Browser): Promise<string | null> {
    return (await browser.execute(function (selector: string) {
        ...
        return (
            el
                ?.getAttribute("data-active-descendant")
                ?.replace(/^history-row-/, "") ?? null
        );
...
export async function gotoView(browser: Browser, label: string): Promise<void> {
    ...
    await browser.waitUntil(
        ...
        {
            timeout: 15_000,
            timeoutMsg: `the ${label} screen never became current`,
        },
    );
}
...
export async function byLabel(
    ...
    Array<{ tag: string; width: number; height: number; text: string }>
> {
...
export async function dismissFirstRun(browser: Browser): Promise<void> {
    await browser.waitUntil(
        async () =>
            browser.execute((primaryNavigation: string) => {
                ...
                const explore = Array.from(
                    ...
                    (button) => button.textContent?.trim() === "Explore first",
                );
