...
describe("Tauri WebDriver capabilities", () => {
    it("accepts WebView2 only on Windows", () => {
        ...
        expect(() =>
            assertTauriBrowserName({ browserName: "wry" }, "win32"),
        ).toThrow(/expected the Tauri WebView \("webview2"\)/);
    });
    ...
    it("retains the wry capability requirement off Windows", () => {
        ...
        expect(() =>
            assertTauriBrowserName({ browserName: "webview2" }, "linux"),
        ).toThrow(/expected the Tauri WebView \("wry"\)/);
    });
    ...
    it.each(["MicrosoftEdge", "chrome", undefined])(
        ...
        (browserName) => {
            expect(() =>
                assertTauriBrowserName({ browserName }, "win32"),
            ).toThrow(/The session is not the app under test/);
        },
    );
});
...
describe("Windows environment probe", () => {
    ...
    it("records compatible versions before the session probe", async () => {
        const directory = mkdtempSync(
            path.join(os.tmpdir(), "cp-windows-probe-test-"),
        );
        ...
        try {
            await probeWindowsEnvironment({
                ...
                driverVersion: async () =>
                    "Microsoft Edge WebDriver 140.0.3485.56",
    ...
    it("classifies a failed compatibility probe as an environment failure", async () => {
        ...
        try {
            const failure = await probeWindowsEnvironment({
                ...
                driverVersion: async () =>
                    "Microsoft Edge WebDriver 140.0.3486.56",
                webview2Version: "140.0.3485.55",
            }).then(
                () => undefined,
                (error: Error) => error,
    ...
    it("fails closed when session cleanup rejects after readiness", async () => {
        ...
        try {
            ...
            expect(written).toContain(
                "tauriSessionProbeCleanupError=driver cleanup failed",
            );
            expect(
                written.lastIndexOf("tauriSessionProbe=failed"),
            ).toBeGreaterThan(written.lastIndexOf("tauriSessionProbe=ready"));
        } finally {
            rmSync(directory, { recursive: true, force: true });
        }
    });
...
describe("Tauri bridge startup", () => {
    ...
    it("rejects another bridgeless page immediately", () => {
        expect(() =>
            assertTauriBridge(
                { bridge: false, url: "http://127.0.0.1:1420/" },
                false,
            ),
        ).toThrow(/no IPC is under test/);
    });
...
describe("WebDriver session failure diagnostics", () => {
    ...
    it("states the elapsed time against the budget before the log", () => {
        ...
        expect(message).toContain(
            "gave up after 120004ms of a 120000ms budget",
        );
...
describe("run manifest", () => {
    const inTemporaryDirectory = async (
        ...
        const directory = mkdtempSync(
            path.join(os.tmpdir(), "cp-manifest-test-"),
        );
        try {
    ...
    it("keeps the environment and the reason when the probe fails", async () => {
        await inTemporaryDirectory(async (manifest) => {
            const failure = await recordRunEnvironment({
                path: manifest,
                probe: () =>
                    Promise.reject(new Error("Add-Type : assembly not found")),
            }).then(
                () => undefined,
                (error: Error) => error,
            );
            ...
            expect(failure?.message).toMatch(
                /could not load System.Windows.Forms/,
            );
            const written = readFileSync(manifest, "utf8");
...
describe("child-process diagnostics", () => {
    it("preserves output and the exit result in the uploaded log", async () => {
        const directory = mkdtempSync(
            path.join(os.tmpdir(), "cp-process-test-"),
        );
        ...
        try {
            const child = track(
                execa(
                    ...
                    [
                        "-e",
                        "console.error('diagnostic marker'); process.exit(7)",
                    ],
                    { reject: false },
                ),
                logPath,
    ...
    it("never overlaps real CLI delete processes", async () => {
        ...
        try {
            ...
            const remainingIds = new Set(
                (await daemon.items()).map((item) => item.id),
            );
