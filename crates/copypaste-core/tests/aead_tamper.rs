...
fn aad_for(item_id: &str) -> Vec<u8> {
    build_item_aad(&item_id.into(), AAD_SCHEMA_VERSION)
}
...
fn aad_swap_fails() {
    ...
    let aad_a = build_item_aad(&"item-A-uuid".into(), 3);
    let aad_b = build_item_aad(&"item-B-uuid".into(), 3);
    ...
    let aad_a_v2 = build_item_aad(&"item-A-uuid".into(), 2);
...
fn aad_match_succeeds() {
    ...
    let aad = build_item_aad(&"item-X-uuid".into(), AAD_SCHEMA_VERSION);
    ...
    let aad_again = build_item_aad(&"item-X-uuid".into(), AAD_SCHEMA_VERSION);
...
fn decrypt_without_aad_fails() {
    ...
    let v3_aad = build_item_aad(&"legacy-row-uuid".into(), AAD_SCHEMA_VERSION);
