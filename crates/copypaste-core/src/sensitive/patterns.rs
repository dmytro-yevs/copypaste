...
pub const RAW_PATTERNS: &[(&str, &str, u8, f32)] = &[
    ...
    ("github_fine_grained",  r"github_pat_[A-Za-z0-9]{22}_[A-Za-z0-9]{59}",                               0, 0.99),
    ...
    ("slack_bot",            r"xoxb-[0-9]{11}-[0-9]{11}-[a-zA-Z0-9]{24}",                                  0, 0.99),
    ("slack_webhook",        r"https://hooks\.slack\.com/services/T[A-Z0-9]+/B[A-Z0-9]+/[a-zA-Z0-9]+",     0, 0.99),
    ("discord_bot_token",    r"[MN][a-zA-Z\d]{23,25}\.[\w-]{6}\.[\w-]{27,38}",                             0, 0.85),
    ("twilio_auth_token",    r"SK[a-f0-9]{32}",                                                              0, 0.90),
    ("google_api_key",       r"AIza[0-9A-Za-z\-_]{35}",                                                     0, 0.99),
    ("heroku_api_key",       r"(?i)heroku[^\n]{0,50}[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}", 0, 0.95),
    ...
    // ── Financial ─────────────────────────────────────────────────────────────
    ("iban",                 r"\b[A-Z]{2}[0-9]{2}[A-Z0-9]{4}[0-9]{7}[A-Z0-9]{0,16}\b",                   1, 0.85),

    // ── Personal Identifiers ──────────────────────────────────────────────────
    ("ssn_us",               r"\b\d{3}[-\s]\d{2}[-\s]\d{4}\b",                                             2, 0.80),
    ("email",                r"\b[A-Za-z0-9._%+\-]+@[A-Za-z0-9.\-]+\.[A-Za-z]{2,}\b",                     2, 0.60),
    ("phone_us",             r"(?:\+1[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}\b",                     2, 0.55),
    ("passport",             r"\b[A-Z]{1,2}[0-9]{6,9}\b",                                                  2, 0.55),

    // ── Infrastructure ────────────────────────────────────────────────────────
    ("ip_with_port",         r"\b(?:(?:25[0-5]|2[0-4]\d|[01]?\d\d?)\.){3}(?:25[0-5]|2[0-4]\d|[01]?\d\d?):\d{2,5}\b", 3, 0.70),
    ("db_conn_string",       r"(?i)(?:postgresql|postgres|mysql|mongodb|redis|amqp|mssql)://[^@\s]*:[^@\s]*@\S+",       3, 0.99),
    ("aws_arn",              r"\barn:aws:[a-z][a-z0-9\-]*:[a-z0-9\-]*:[0-9]{12}:[^\s]+",                   3, 0.90),
    ("dotenv_secret",        r"(?m)^(?:export\s+)?[A-Z][A-Z0-9_]{2,}(?:_KEY|_SECRET|_TOKEN|_PASSWORD|_PASS|_PWD|_CREDENTIALS?)\s*=\s*\S+", 3, 0.80),
];

pub fn pattern_set() -> &'static RegexSet {
    PATTERN_SET.get_or_init(|| {
        RegexSet::new(RAW_PATTERNS.iter().map(|(_, p, _, _)| *p)).unwrap()
    })
}

pub fn patterns() -> &'static Vec<Regex> {
    PATTERNS.get_or_init(|| {
        RAW_PATTERNS.iter().map(|(_, p, _, _)| Regex::new(p).unwrap()).collect()
    })
}

pub fn pattern_name(index: usize) -> &'static str {
    RAW_PATTERNS.get(index).map(|(n, _, _, _)| *n).unwrap_or("unknown")
}
...
pub fn pattern_confidence(index: usize) -> f32 {
    RAW_PATTERNS.get(index).map(|(_, _, _, conf)| *conf).unwrap_or(0.5)
}
