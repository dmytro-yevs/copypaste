//! Integration tests for the copypaste-android FFI surface.
//!
//! All tests live here (extracted from lib.rs) so the god-file stays under
//! ~80 lines of glue.  `use super::*` brings in every re-exported symbol from
//! `lib.rs`, so the test surface is unchanged.
...
fn encrypt_then_decrypt_roundtrips() {
    ...
    let plaintext = decrypt_text(item_id, &blob.ciphertext, &blob.nonce, &key, 2).expect("decrypt");
...
fn db_vacuum_succeeds_as_noop_when_feature_off() {
    ...
    db_vacuum("/dev/null".into(), &test_key()).expect("stub db_vacuum must succeed without I/O");
}
...
fn sync_with_peer_receives_image_frame_from_loopback_peer() {
    ...
    let known_plaintext: Vec<u8> = vec![0x89, b'P', b'N', b'G', 0x0D, 0x0A, 0x1A, 0x0A, 1, 2, 3];
    let known_blob = encrypt_for_cloud(&shared, &known_item_id, &known_plaintext)
