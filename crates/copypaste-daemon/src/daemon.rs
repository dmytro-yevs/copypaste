...
pub async fn run_with_quit_flag(quit_flag: Arc<AtomicBool>) -> anyhow::Result<()> {
    ...
    // Acceptance criteria #1 + #2: bound the Keychain read so we can never hang
    // on a GUI prompt, then decide the DB-open plan from the key outcome and
    // whether real encrypted data already exists. When the key is unavailable
    // AND an encrypted DB exists, we MUST NOT fall back to an ephemeral key
    // against that DB (that yields SQLITE_NOTADB and a dead daemon); instead we
    // go DEGRADED — alive, socket bound, recovery status served, DB untouched.
    ...
    // The actual key to hand subsystems. For `Open` it is the real key just
    // read; for `OpenEphemeral` it is a fresh ephemeral key (no real data to
    // protect); the `Degraded` path never reaches the key-using subsystems.
    let local_key_arc: Arc<zeroize::Zeroizing<[u8; 32]>> = match &plan {
        DbStartupPlan::Open => match key_load {
            KeyLoad::Ready(k) => Arc::new(k),
            // unreachable: `Open` is only produced for `Ready`.
            KeyLoad::Locked => unreachable!("Open plan implies a Ready key"),
        },
        DbStartupPlan::OpenEphemeral => Arc::new(DeviceKeypair::generate().local_enc_key()),
        DbStartupPlan::Degraded { reason } => {
            // Safety net for the post-reinstall regression: do NOT open, write,
            // or recreate the existing encrypted DB (acceptance criterion #3).
...
fn decide_db_startup(key: &KeyLoad, encrypted_db_exists: bool) -> DbStartupPlan {
    match key {
        KeyLoad::Ready(_) => DbStartupPlan::Open,
...
fn load_local_key_bounded() -> KeyLoad {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if crate::keychain::keychain_bypassed() {
            return KeyLoad::Ready(load_local_key());
        }

        let (tx, rx) = std::sync::mpsc::sync_channel::<zeroize::Zeroizing<[u8; 32]>>(1);
...
mod tests {
    ...
    fn ready_key() -> KeyLoad {
        KeyLoad::Ready(zeroize::Zeroizing::new([0x11u8; 32]))
    }
