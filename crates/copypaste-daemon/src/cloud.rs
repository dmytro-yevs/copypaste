...
pub async fn start_cloud(
    ...
    let bearer_str = resolve_bearer_with_client(&config, &auth_client).await?;
    ...
    loop {
        ...
        match push_item_once(client, url, &config.anon_key, &token, item, payload_ct_b64).await {
            ...
            PushOutcome::Unauthorized if !refreshed_once => {
                ...
                match refresh_bearer(config, auth).await {
                    Ok(new_token) => {
...
async fn refresh_bearer(config: &CloudConfig, auth: &AuthClient) -> Result<String, String> {
    ...
    resolve_bearer_with_client(config, auth)
        .await
        .map_err(|e| e.to_string())
}
    ...
    loop {
        tokio::select! {
            _ = interval.tick() => {
                ...
                match fetch_remote_rows_with_refresh(&client, &poll_url, &config, &bearer, &auth).await {
...
mod tests {
    ...
    async fn push_during_disconnect_retries_on_reconnect() {
        ...
        let result = tokio::time::timeout(
            ...
            push_item_with_retries(&client, &url, &cfg, &bearer, &item, "dGVzdA==", &auth),
        )
        .await
