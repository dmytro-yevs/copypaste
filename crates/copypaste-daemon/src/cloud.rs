...
pub async fn start_cloud(
    ...
    let ws_history_limit = app_config.history_limit;
    let ws_quota_bytes = app_config.storage_quota_bytes;
...
// ── WebSocket ingest loop (v0.5.3) ────────────────────────────────────────────

/// Background task: maintain the Supabase Realtime WebSocket connection and
/// ingest INSERT events as they arrive, running the same decrypt → LWW →
/// dedup → re-encrypt → insert → prune pipeline as the HTTP poll loop.
///
/// # Architecture
///
/// ```text
/// Supabase Realtime WS
///   └─ postgres_changes INSERT → ws_ingest_loop → same ingest path as poll_once
///
/// HTTP poll loop (realtime_loop)
///   └─ ticks slowly (120 s) when ws_connected=true   ← catch-up / safety net
///   └─ ticks fast  (10  s) when ws_connected=false   ← sole download path
/// ```
///
/// # Token refresh
///
