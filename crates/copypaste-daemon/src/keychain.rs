...
///
/// On first run (or after `delete_stored`), the keypair is written with
/// `kSecAttrAccessibleWhenUnlockedThisDeviceOnly` + `kSecAttrSynchronizable=false`
/// so the secret never leaves the device via iCloud Keychain sync or a
/// Time Machine backup of the System Keychain (audit HIGH #2).
///
/// On subsequent runs, if the existing item was written by an older build
/// using the default `WhenUnlocked` accessibility (cloud-sync-eligible),
/// it is opportunistically re-written with the locked-down ACL — see
/// `migrate_legacy_accessibility_if_needed` below.
    ...
    #[cfg(target_os = "macos")]
    {
        match get_generic_password(SERVICE, ACCOUNT) {
            ...
            Err(_) => {
                ...
                // Audit MED #3 + #4: pull the secret via the zeroizing
                // accessor so the buffer handed to the Keychain syscall
                // is scrubbed when this function returns.
                let secret = kp.secret_key_bytes_zeroizing();
                set_generic_password_locked_down(SERVICE, ACCOUNT, secret.as_ref())?;
