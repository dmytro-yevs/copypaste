...
//!
//! Two things this adds to [`transport::Session`], both of which the sync
...
use copypaste_p2p::protocol::SyncMessage;
use copypaste_p2p::sync::{SyncChannel, SyncError};
...
mod tests {
    ...
    async fn messages_round_trip_through_encode_and_decode() {
        ...
        a.send(SyncMessage::Hello {
            protocol_version: copypaste_p2p::PROTOCOL_VERSION,
    ...
    async fn a_message_over_the_wire_bounds_is_refused_before_it_is_sent() {
        ...
        let items = std::iter::repeat_with(|| copypaste_p2p::protocol::SyncItem {
            item_id: "x".into(),
            content: String::new(),
        ...
        .take(copypaste_p2p::protocol::MAX_ITEMS_PER_MESSAGE + 1)
        .collect();
