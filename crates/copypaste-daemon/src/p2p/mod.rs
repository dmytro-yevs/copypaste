...
use std::sync::Arc;
// `Instant` is only exercised by the RTT-tracking tests below.
#[cfg(test)]
...
// `PeerFrame` (bare) is only exercised by the tests below; production code
// always goes through fully-qualified `copypaste_sync::protocol::PeerFrame`.
#[cfg(test)]
use copypaste_sync::protocol::PeerFrame;
