...
mod tests {
    ...
    /// The single cloud-sync key persists to its own file and round-trips:
    /// absent until written, reloads the exact bytes, and survives a restart
    /// (the daemon reloads these bytes without re-entering the passphrase).
    #[cfg(feature = "cloud-sync")]
    #[test]
    fn cloud_sync_key_round_trips() {
        let _env = KeyFileEnv::new();
        // Nothing persisted yet.
        assert_eq!(
            load_cloud_sync_key().expect("load (absent)"),
            None,
            "no cloud-sync key until a passphrase is set"
        );

        let secret = [0x22u8; 32];
        store_cloud_sync_key(&secret).expect("store");
        assert_eq!(
            load_cloud_sync_key().expect("load"),
            Some(secret),
            "the cloud-sync slot must reload the exact bytes (restart-surviving)"
        );
    }
