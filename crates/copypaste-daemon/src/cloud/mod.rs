...
// ── Single per-account sync key snapshot ────────────────────────────────────

/// Snapshot the single per-account cloud sync key as a 32-byte array, or `None`
/// when no passphrase is set (the caller then skips upload/download).
///
/// There is exactly one key — the per-account-salt key shared by the cloud
/// (Supabase) and relay paths — so reads and writes use the same bytes. The
/// lock is taken and immediately released; the returned bytes are the caller's
/// responsibility to zeroize.
pub(crate) async fn snapshot_cloud_key_bytes(
    sync_key: &std::sync::Arc<tokio::sync::Mutex<Option<copypaste_core::SyncKey>>>,
) -> Option<[u8; 32]> {
    let g = sync_key.lock().await;
    g.as_ref().map(|k| *k.as_bytes())
}
