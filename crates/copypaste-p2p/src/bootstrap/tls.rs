//! rustls certificate verifier that accepts any peer certificate without pinning.
//!
//! Used only on the bootstrap channel. TLS is still used (for encryption in
//! transit and so the peer's cert fingerprint can be observed), but the
//! chain/expiry/hostname/fingerprint validation is intentionally skipped.
//! Authentication is provided by the PAKE handshake.
