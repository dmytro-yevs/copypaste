...
import { COPY_FLASH_MS, OVERSCAN_ROWS, rowHeight } from "../lib/layout";
import { HistoryRow, rowLabel } from "./HistoryRow";
...
export function HistoryList({
  ...
  const virtualizer = useVirtualizer({
    ...
    // INV-5: a constant reservation, not a function of the text.
  ...
  function select(index: number) {
    ...
    // Scrolling comes from the height model, not scrollIntoView: the target
    // row may not be in the DOM at all (manifest §3.1.4).
  ...
  function onKeyDown(event: KeyboardEvent<HTMLDivElement>) {
    ...
    switch (event.key) {
      case "ArrowDown":
        ...
        // Clamped at both ends; the history list does not wrap.
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col">
      <div
        ...
        className="min-h-0 flex-1 overflow-x-hidden overflow-y-auto px-s-4 pb-s-4 focus-visible:-outline-offset-2"
      >
        <div
          role="list"
