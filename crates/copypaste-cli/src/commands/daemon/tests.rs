...
impl CommandRunner for MockRunner {
    fn run(&mut self, program: &str, args: &[std::ffi::OsString]) -> anyhow::Result<CommandOutput> {
...
impl MockFs {
    ...
    #[allow(dead_code)] // builder helper, not called by every test
    fn with_file(mut self, p: impl Into<std::path::PathBuf>, content: impl Into<String>) -> Self {
        let p = p.into();
...
fn daemon_uninstall_removes_leftover_socket() {
    let socket =
        std::path::PathBuf::from("/Users/test/Library/Application Support/CopyPaste/daemon.sock");
    let mut runner = MockRunner::new();
    runner.set_response("launchctl", "print", true, "", "");
...
fn daemon_uninstall_does_not_disable_label() {
    ...
    let disable_called = runner
        .calls
        .iter()
        .any(|c| c.program == "launchctl" && c.args.first().map(|s| s.as_str()) == Some("disable"));
    assert!(
        !disable_called,
...
fn error_5_translates_to_bootstrap_failed_with_enable_hint() {
    ...
    let msg = friendly_launchctl_error(501, "bootstrap", "Bootstrap failed: 5: Input/output error");
    assert!(
