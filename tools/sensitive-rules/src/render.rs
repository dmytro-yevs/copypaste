...
pub fn render(inputs: &Inputs) -> Result<String> {
    ...
    writeln!(out, "// @generated by `cargo run -p copypaste-sensitive-rules -- generate`.")?;
    writeln!(out, "// Source: Gitleaks {} ({})", source.release, source.commit)?;
    writeln!(out, "// Do not edit this file; edit config/sensitive-rules.toml.")?;
    writeln!(out)?;
    writeln!(
        out,
    ...
    writeln!(out, "pub(super) static SELECTED_GITLEAKS_RULE_IDS: &[&str] = &[")?;
    for id in &inputs.selected_ids {
        writeln!(out, "    {},", rust_string(id))?;
    }
    writeln!(out, "];")?;
    writeln!(out)?;
    writeln!(out, "pub(super) static GLOBAL_ALLOWLISTS: &[AllowlistSpec] = &[")?;
...
fn constant(out: &mut String, name: &str, value: &str) -> Result<()> {
    writeln!(out, "#[cfg(test)]")?;
    writeln!(out, "pub(super) const {name}: &str = {};", rust_string(value))?;
    Ok(())
}
...
fn render_rule(out: &mut String, rule: &Rule) -> Result<()> {
    ...
    writeln!(out, "        category: Category::{},", category(rule.category))?;
    writeln!(out, "        confidence: {:?},", rule.confidence)?;
    writeln!(out, "        pattern: {},", rust_string(&rule.pattern))?;
    writeln!(out, "        validator: Validator::{},", validator(rule.validator))?;
