...
| The pull query gets an index scan with no sort | **verified** — `tests/03`, 10,000 rows |
...
| Per-item size (10 MiB image/file, 8 MiB text) | **Client**, with a server backstop | Manifest verdict for row 13: enforce before upload so the user gets a clear local error rather than an opaque backend rejection. The server's `check (octet_length(ciphertext) <= 16 MiB)` sits above every client-side limit; it bounds abuse, it is not the product limit. **The client-side split caps are still owed.** |
...
| 6 | Keyset cursor pagination with no ties | Client cursor + the `(user_id, created_at, item_id)` index | 🟡 **Covered with a weaker cursor than v1's.** See the note below — this is the one row where I think something was lost in the migration and it is client-side. |
...
| 13 | Per-item size cap, split by type | Server `check` at 16 MiB base64 (backstop) | ⚠️ **Still owed client-side.** The manifest's verdict is "move client-side, keep the split limits (10 MiB image/file, 8 MiB text)" so the user gets a local error. The backstop is not that. |
...
### Row 6, in full — the one I think is still open
...
v2's client sends a single inclusive bound — `created_at=gte.<cursor>` — ordered
`created_at.asc,item_id.asc`. The inclusive bound is right and fixes the worst
half of the v1 bug: no row sharing the boundary millisecond is skipped, because
re-offering a row is free and skipping it is not. But the cursor still carries
only the millisecond, so if **more than one page of rows shares a single
`created_at`** (100 rows, `PULL_PAGE_LIMIT`), the watermark cannot advance past
that millisecond: every pull re-fetches the same first 100 of them by `item_id`
and the rest are never reached. `pull.rs`'s stall guard turns that from a spin
into a warning and a break, so it is loud rather than silent — but the rows
behind it do not download.

That needs one page-worth of items stamped inside one millisecond, which is a
bulk import or a backfill push, not ordinary typing. I am flagging it rather
than fixing it because the fix is in `crates/copypaste-cloud` (a second cursor
component, `item_id`, and an `or=(…)` filter) and this deployment already has
the index that a compound keyset would need — `(user_id, created_at, item_id)`
serves both shapes.
...
One factual disagreement, in the client's prose rather than its behaviour:
`rest/mod.rs` describes the read index as "the `(created_at asc, item_id desc)`
keyset this module orders on", while `fetch_since` two screens further down
sends `order=created_at.asc,item_id.asc`, and `sync::pull::sort_page` sorts
ascending on both. The code is consistent and the comment is stale; the index
here matches the code. Building the index the comment describes would cost an
incremental sort on every page.

## What is not here

- **Pin state does not cross this transport.** Manifest 05 T-6 and §3.6 treat
  `pinned`/`pin_order` as ordinary LWW fields that must travel, and v2's
  `CloudItem` has no such columns — so the table has none either. Adding columns
  the client never writes would be inventing a shape rather than serving one.
  Whether pinning is expected to sync through P2P only, or is an accepted loss,
  is a client-side decision that has not been written down anywhere I can find.
