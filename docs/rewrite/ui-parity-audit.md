...
At the time of writing, a known bug meant v2's UI did not render in a real
browser; nothing below depended on having run it. It has since been fixed —
`e2e/` drives the built app through a real WebKitGTK WebView, 74 tests — but
none of the verdicts here were revisited against it.
...
| **9** | **"Too large to sync" on a row.** v1 drew a warning icon, visible even in selection mode (`CopyPaste-f72f`), on any item over the sync size cap. | **Bridge half closed** — `UiItem::too_large_to_sync` (`src-tauri/src/model.rs`), and the round-level counterpart `CloudSyncData::skipped_too_large`. Not consumed: `src/lib/ipc.ts`'s `Item` does not declare it and `HistoryRow.tsx` draws nothing. An item that will silently never reach the other device still looks exactly like one that will. | v2 `src-tauri/src/model.rs`; `src/lib/ipc.ts` `Item`; `HistoryRow.tsx`. v1 `views/HistoryView/HistoryRow.tsx:315-325`. |
...
| Drag-to-reorder pinned items | **⚠ Missing at the screen** — everything under it landed (`Store::reorder_pinned`, `Method::ReorderPinned`, CLI `reorder`, a `useReorderPinned` hook). No drag affordance, and both Tauri backends refuse the call. `parity-audit.md` finding 19. |
...
| **Sync** | **Reduced to a status page** — paired count, Sync now, "Cloud sync: Unavailable". LAN visibility and the sync toggle since landed on a **Service** tab (`ServiceTab.tsx` over `useServiceConfig`), which also carries poll interval, history limit, retention days, max item bytes, sensitive TTL, notify and sound — `parity-audit.md` finding 9's UI half. Credentials, passphrase, wifi-only and auto-apply are still absent. |
...
| Localisation | **Closed since this audit** — `src/i18n/`, one catalogue behind i18next, with `catalogue.test.ts` over it. English only, but no user-facing string is hard-coded in a component. v1 shipped `values-uk` on Android only, so no desktop UI string was ever translated. |
