...
| Sensitive TTL (default) | **30 s** | Long enough to paste a password once. Settings can set `0` to disable. Auto-wipe floor is **0.85**. `copypaste_ipc::ConfigData::sensitive_ttl_secs` |
| Autowipe confidence floor | **0.85** | Classify floor stays **0.70**. Below 0.85, generic/KV noise does not auto-wipe. `sensitive/finding.rs` |
