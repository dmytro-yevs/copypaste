...
| Android app | **Landed, unfinished** — the app is scaffolded, `backend/embedded.rs` is the in-process backend, the capture ladder and its UI exist (ADR-0005), the Keystore backend exists, and the release builds an APK (ADR-0006). Seven operations still return `Unsupported`: reorder, both pairing verbs, sync, discovered, rescan, watch. Never compiled — no NDK. |
