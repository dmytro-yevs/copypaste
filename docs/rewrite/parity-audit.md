...
| **9** | **User-facing settings of any kind** | **Closed on the wire** — `ipc/src/config.rs`, `daemon/src/settings.rs`, `Method::GetConfig` / `SetConfig`, CLI `config show` / `config set`. No Settings tab reads it. v1's `AppConfig` had 21 fields (poll interval, size caps, storage quota, sensitive TTL, exclusion list, sound/notify on copy, LAN visibility, sync toggles, `paste_as_plain_text`, …), reachable over `get_config`/`set_config` and a five-tab Settings screen. v2 has no config file, no config IPC, no Settings UI; every constant is compiled in. | `git show archive/…:crates/copypaste-core/src/config/mod.rs`; v2 has no `config` module and `daemon/src/main.rs` takes four CLI flags. Not in README "Not built". |
