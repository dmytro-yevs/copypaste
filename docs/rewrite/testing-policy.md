...
| Android | `android-emulator.yml` on x86_64, including `e2e-android/` over CDP; `release.yml` emulator smoke of the signed APK | Android platform behavior at each API level that ran; the debug WebView document; the signed release APK on the emulator | macOS; OEM background policy; a Play-updated WebView; physical arm64 (`NOT VERIFIED IN CI`) |
