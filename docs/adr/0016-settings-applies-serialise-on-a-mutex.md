# ADR-0016: Serialise settings applies on a mutex, not on the read lock
...
the server modules. That wide diff is not justified for a low-rated finding.
Reconsider it if the read side becomes hot; nothing here forecloses it.
