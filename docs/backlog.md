...
|---|---|---|---|---|
| **B-33** | **The Android build answers "no v0.4 history" without looking.** | `status_of` hard-codes `legacy_history_present: false` on the reasoning that v0.4's Android history was inside the app sandbox and `copypaste_ipc::v1_data_dir()` cannot address it. The first half is true and the conclusion does not follow: this build *is* inside a sandbox, and it already knows which one — `EmbeddedBackend::open` is handed it. An Android user upgrading in place gets the empty history and the silence rule 3 forbids, and unlike the desktop case nothing else on the screen says why. | `ui/src-tauri/src/backend/embedded/rows.rs` (`status_of`) and `mod.rs` (`Inner` does not keep `data_dir`, which is why the probe has nothing to ask). Whether v0.4's Android package wrote `clipboard.db` into the same sandbox is unrecorded in any manifest — the probe is honest either way, because `v1_database_in` answers on v0.4's schema or its exact filename and page shape, and v2's own directory holds neither. | Nothing. Keep `data_dir` on `Inner`; `legacy_history_present: copypaste_core::v1_database_in(&inner.data_dir)`. Unrunnable here (§6) |
