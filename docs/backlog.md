...
| post-merge 2 · ui-parity 2 · B-4 | The v0.4 detector was never asked | `daemon/src/main.rs`'s startup probe over `data_dir` **and** `copypaste_ipc::v1_data_dir()`, into `StatusData::legacy_history_present`, the CLI note (`render/service.rs`) and `ui/src/lib/banners.ts`. Verified end to end on 2026-07-31 against staged v0.4 files, plaintext and SQLCipher-shaped: flag true, CLI says so, file byte-identical by sha256 with mtime unchanged and no `-wal`/`-shm`/`-journal`. Android is still `false` — B-33 |
| parity D8 · AT-33 · B-5 | Mutations during a backend outage were thought unqueued | **The finding was wrong about the mechanism and right about one writer.** Push re-derives from `versions_since(upload_floor)`, and `poll::sync_round` advances the floor only on a completed round, so an outage costs latency: run 2026-07-31, two captures and a delete made with the backend down, daemon restarted mid-outage, all three uploaded on the first round after recovery and nothing older re-uploaded. The real hole was `server/transfer.rs::import`, which kept the export's `created_at` and never lowered the floor — imported history was stored, listed and silently never uploaded. Fixed, plus `KEY_LAST_SYNC` persisted so a restart mid-outage no longer reports `last sync never`. Manifest 05 §4.9 amended |
| B-1 · parity D6 | Keyset pagination existed in `copypaste-core` and was on no wire | `Method::List { limit, cursor }` and `ItemPage::next_cursor` over `Store::list_from`; both Tauri backends, `commands::history::list`, `lib/ipc.ts`, `useHistory`, and CLI `list --cursor`. `--offset` is gone from every surface rather than merely unused. The cursor carries the whole sort key by value (`pinned`, `pin_order`, `created_at`, `id`), so pinning or deleting the anchor cannot strand a reader. `Search` stays unpaged, stated on `Method::Search`: FTS5 `rank` is a score, not an order to seek on |
| B-2 · sec F-3 | Peer-supplied `content_hash` was stored and ordered on | A live version's hash is recomputed in `apply_remote_version` whatever the transport claims, so the trusted field is tombstone-only — `crates/copypaste-core/src/sync/merge.rs`. The peer session already dropped a lying live item (`session.rs`); this closes the same gap for a caller that is not that session. The tombstone exemption is bounded rather than tolerated: `deleted` is merge key 3, so a forged hash deletes exactly what an honest one would or deletes *less*, never more (`a_forged_tombstone_hash_can_only_delete_less_than_an_honest_one`) |
| B-33 · post-merge 2 (Android half) | The Android build answered "no v0.4 history" without looking | `Inner` keeps `data_dir` and `rows.rs`'s `legacy_history_present` asks `copypaste_core::v1_database_in` about the sandbox this build was handed. **Three directories, not the one the fix above named:** Tauri's `app_data_dir()` is `Context.dataDir`, the sandbox *root*, and an Android app puts a database under `databases/` or `files/` — a probe of the root alone would answer "no history" for exactly the case this item is about. Read-only, and asserted so: `a_v0_4_history_anywhere_in_the_sandbox_is_reported_and_left_untouched` compares the file byte-for-byte before and after in each of the three and fails on any `-wal`/`-shm`/`-journal`. **The true-positive case is unverified** — no manifest records which directory v0.4's Android package used, or whether its `applicationId` matched this one's, and nothing here can run an Android build. What the tests do pin is the false-positive side, which is the one that would reach every user: `a_directory_this_build_made_itself_is_not_a_v0_4_history` |
| B-6 · parity S11 · INV-35 | Screen-capture protection was off | On by default and turned off only by a setting the user can find. `contentProtected: true` in `tauri.conf.json` protects the desktop window from creation; `FLAG_SECURE` in `MainActivity.onCreate` does the same on Android, where `set_content_protected` is a no-op (tao gates it to macOS and Windows). `shell/protection.rs` + `commands::protection::set_allow_screenshots` carry the opt-out down, `ScreenProtectionPlugin.kt` applies it on Android, and `settings.list.allowScreenshots` is where the user reaches it. The reveal path needs no ordering against any of it: protection is on before the first frame, and every failure path leaves it on. Never executed on either platform (§6) |
...
**Empty.** B-33 was the only entry and it closed; see §1.
...
| # | Item | What the user loses | Evidence | Depends on |
|---|---|---|---|---|
| **B-11** | **`reset_database` and `vacuum` have no verb.** | The escape hatch B-33 needs, and the only way to recover from an unopenable store without deleting a file by hand — which no error may name (rule 4). | `Method` has `Backup` / `Restore`, not these. `daemon/src/server/dbadmin.rs`. Parity 6 (residue). | Nothing. |
...
| **A `BackendError` a screen can name** | Every future condition a retry cannot fix | `StoreError::LegacyDatabase` is the one that already needed it, and it reached the surface another way — the startup probe and `StatusData::legacy_history_present`, not the error path |

B-11 is still what B-33 would want for an escape hatch, on the same reasoning
B-4 wanted it: a user told their old history is there has nowhere to go next.
...
|---|---|---|
| **A Mac** | `NSWindow.sharingType = .none` — that `contentProtected` and `set_content_protected` actually keep the window out of a recording, and what a shared screen shows in its place. `NSPasteboard` capture, the Keychain device-secret store, tray placement and the popover landing under the icon, the global hotkey, launch-at-login, whether `Contents/MacOS/copypaste-daemon` starts from inside a quarantined ad-hoc-signed bundle, whether `codesign` / `hdiutil` / the Tauri bundler work, whether the cask's `xattr` + re-seal is sufficient, and **B-31** — whether TCC accepts a self-signed certificate | ADR-0003 §"Verification status"; ADR-0004 §"Consequences"; ADR-0001 §"Verified, 2026-07-30"; `README.md` **Unverified** |
| **An Android device + SDK** | `FLAG_SECURE`: that `MainActivity` sets it before the first frame, that `ScreenProtectionPlugin.setProtected` clears and re-adds it on a live window, and what the recents thumbnail looks like while it is on. And B-33's true-positive half — whether v0.4's Android package wrote `clipboard.db` into the directory `app_data_dir()` resolves to. The whole ladder. The central claim — that a binder call as the shell uid reads the clipboard with no focus — is derived from AOSP source and has never been observed. Also: whether the listener registers and fires as shell, whether `mAppOps.checkPackage(2000, "com.android.shell")` passes through Shizuku, what the Android 12+ toast looks like in practice, and OEM variance in on-device wireless-debugging pairing | `android-clipboard-access.md` §4 (marked **blocking**) and Open questions; `src-tauri/src/capture/mod.rs` §Unverified |
...
`e2e/tests` (74). Two more on 2026-07-31, both against running daemons: the
cloud outage and import rounds behind the closed B-5 row, and the v0.4 probe
behind the closed B-4 row. Re-check `git status` before starting anything. The audits
