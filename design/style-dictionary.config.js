...
const ANDROID_RES = '../crates/copypaste-ui/src-tauri/gen/android/app/src/main/res/';

const androidName = (token) => `copypaste_${token.path.slice(2).join('_').replaceAll('-', '_')}`;

const androidColor = (value, vars) => {
  const hex = formatHex8(resolve(value, vars));
  if (!hex) throw new Error(`cannot write Android colour: ${value}`);
  const rgb = hex.slice(1, 7).toUpperCase();
  const alpha = hex.slice(7, 9).toUpperCase();
  return `#${alpha}${rgb}`;
};

const androidDimension = (token) => {
  const value = tokenValue(token);
  const match = /^(\d+(?:\.\d+)?)px$/.exec(value);
  if (!match) throw new Error(`Android dimension ${token.path.join('.')} is not pixels: ${value}`);
  return `${match[1]}${ext(token).androidUnit ?? 'dp'}`;
};

StyleDictionary.registerFormat({
  name: 'android/copypaste-resources',
  format({ dictionary, options }) {
    const tokens = dictionary.allTokens
      .filter((token) => token.path[0] === 'android' && token.path[1] === options.group)
      .sort((a, b) => a.path.join('.').localeCompare(b.path.join('.')));
    if (!tokens.length) throw new Error(`no Android ${options.group} tokens`);

    const vars = recipeVars(dictionary.allTokens);
    const element = options.group === 'color' ? 'color' : 'dimen';
    const value = options.group === 'color'
      ? (token) => androidColor(tokenValue(token), vars)
      : androidDimension;
    const entries = tokens.map((token) => `    <${element} name="${androidName(token)}">${value(token)}</${element}>`);
    return [
...
const androidPlatform = (theme) => ({
  buildPath: ANDROID_RES,
  files: [
    {
      destination: theme === 'dark' ? 'values-night/colors.xml' : 'values/colors.xml',
      format: 'android/copypaste-resources',
      filter: (token) => token.path[0] === 'android' && token.path[1] === 'color',
      options: { group: 'color' },
    },
    ...(theme === 'dark' ? [] : [{
      destination: 'values/dimens.xml',
      format: 'android/copypaste-resources',
      filter: (token) => token.path[0] === 'android' && token.path[1] === 'dimension',
      options: { group: 'dimension' },
    }]),
  ],
});
  ...
  return [
    {
      ...
      format: 'css/copypaste',
      filter: (t) =>
        !isThemed(t) && !inGroup(t, 'tw', 'themes', 'recipe', 'android') && targets(t).includes('css'),
      options: {
