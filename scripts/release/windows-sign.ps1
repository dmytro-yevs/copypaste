...
function Invoke-SelfTest {
    ...
    $oversizedSignatureTarget = "$testPfx.oversized.dll"
    ...
    try {
        ...
        $oversizedSignatureBytes = [IO.File]::ReadAllBytes(
            [System.Management.Automation.PSObject].Assembly.Location)
        $peHeaderOffset = [BitConverter]::ToInt32($oversizedSignatureBytes, 0x3c)
        ...
        try {
            ...
            if ($_.Exception.Message -cne "Embedded Authenticode signature has an invalid size") { throw }
            $oversizedSignatureRejected = $true
        }
        if (-not $oversizedSignatureRejected) {
            throw "embedded signature size limit self-test failed"
        }
        ...
        Remove-Item -LiteralPath $oversizedSignatureTarget -Force -ErrorAction SilentlyContinue
