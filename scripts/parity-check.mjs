 ...
 * Returns { accent, success, warning, danger, bg, panel, elevated } as [r,g,b].
 *
...
function parseIdeColorsFields(block, symbols) {
  const out = {};
  const fields = ["accent", "success", "warning", "danger", "bg", "panel", "elevated"];
 ...
 */
const DARK_TOKENS = ["accent", "bg0", "bg1", "bg2", "success", "warning", "danger"];
const LIGHT_TOKENS = ["accent", "bg0", "bg1", "bg2"];
...
const PALETTE_CHECKS = [
  ...
  // liquid-blue: accent only (no named IdeColors — uses DarkIdeColors)
  { palette: "liquid-blue",   cssTheme: "dark", ktKey: "liquid-blue",  tokens: ["accent", "bg0"] },
];
...
function main() {
  ...
  for (const { palette, cssTheme, ktKey, tokens } of PALETTE_CHECKS) {
    ...
    for (const token of tokens) {
      ...
      if (!cssVal) {
        warnings.push(
          `  WARN  [${palette}][${cssTheme}] ${token}: missing from CSS (Android: ${fmtRgb(ktVal)})`
        );
        totalChecks--;
        continue;
      }
      if (!ktVal) {
        warnings.push(
          `  WARN  [${palette}][${cssTheme}] ${token}: missing from Kotlin (CSS: ${fmtRgb(cssVal)})`
        );
        totalChecks--;
        continue;
      }

      const ok = withinTolerance(cssVal, ktVal);
      ...
      if (ok) {
        passedChecks++;
        if (VERBOSE) {
          console.log(`  PASS  ${label}: CSS=${cssHex} Android=${ktHex}`);
        }
      } else {
        failures.push(
          `  FAIL  ${label}:\n` +
          `          CSS    = ${cssHex} (rgb ${cssVal.join(",")})\n` +
          `          Android= ${ktHex} (rgb ${ktVal.join(",")})\n` +
